				...
				<form action="POST" class="contacts-form" id="contacts" style="--border: url('./img/form-border.svg')" v-on:submit.prevent="onSubmit">
					...
					<div class="contacts-form__wrapper">
						<div class="contacts-form__column">
							<label class="contacts-form__label" :class="{ 'contacts-form__label--full': inputs.name.length > 0 }">
								<p class="contacts-form__text">Представьтесь, пожалуйста<span>*</span></p>
								<input type="text" class="contacts-form__input" v-model="inputs.name" required />
							</label>
							<label class="contacts-form__label" :class="{ 'contacts-form__label--full': inputs.email.length > 0 }">
								<p class="contacts-form__text">Эл. почта для ответа<span>*</span></p>
								<input type="email" class="contacts-form__input" v-model="inputs.email" required />
							</label>
						</div>
						<div class="contacts-form__column">
							<label class="contacts-form__label" :class="{ 'contacts-form__label--full': inputs.about.length > 0 }">
								<p class="contacts-form__text">О Вашем проекте</p>
								<input type="text" class="contacts-form__input" v-model="inputs.about" />
							</label>
							<label class="contacts-form__label" :class="{ 'contacts-form__label--full': inputs.phone.length > 0 }">
								<p class="contacts-form__text">Телефон<span>*</span></p>
								<input type="tel" class="contacts-form__input" v-model="inputs.phone" required />
							</label>
						</div>
						<div class="contacts-form__row">
							<label class="checkbox">
								<input type="checkbox" class="checkbox__input" required v-model="inputs.checked" />
								...
								<p class="checkbox__text">Я ознакомлен с <router-link to="/policy">политикой обработки персональных данных</router-link> и согласен на их обработку</p>
							</label>
							<button class="contacts-form__submit" type="submit" :disabled="!inputs.checked || inputs.name === '' || inputs.email === '' || inputs.phone === '' || disabled">
								Отправить
							</button>
						</div>
					</div>
...
export default {
	...
	mounted() {
		...
		this.$data.map = new mapgl.Map('contacts-map', {
		...
		this.$data.offices.forEach((office) => {
			mapgl.marker(office).addTo(this.$data.map);
			mapgl.marker(office).addTo(this.$data.map);
		});
	},
	methods: {
		...
		goTo(address) {
			...
			this.$data.map.panTo(this.$data.offices[address]);
		},
	},
};
</script>
